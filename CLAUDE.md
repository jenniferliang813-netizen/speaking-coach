...
- `index.html` 完工並驗收通過（sonnet 實作、Fable 5 逐項驗收）。麥克風鈕支援「按住說話」與「點一下切換」雙模式（350ms 閾值）。
- 2026-07-03 晚：PWA 化＋部署 GitHub Pages。file:// 收不到聲音 → 根因瀏覽器封鎖，線上版解決。
- 2026-07-04：Android 辨識重複 bug 修復史詩（詳見坑 5/6）——最終 v7 實機結案。**過程教訓：使用者回報「還是壞的」時，先確認手機跑的版本號，再懷疑邏輯**（v6 邏輯其實已修對，卡的是 PWA 快取）。
- 手機練習紀錄：桌面 PWA 與手機 Chrome 同源共用 localStorage，紀錄互通；但與電腦不互通（各裝置獨立）。
- `.claude/launch.json` 是測試用 dev server 設定（`py -m http.server 8123`），日常使用不需要它。
- 錄音回放：手機一律停用（坑 3）；桌機保留但**未經真人驗證**。
