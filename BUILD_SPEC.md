...
3. **對話回合**（每情境 3 回合）：
   ...
   - 使用者**按住麥克風鈕說話**（放開結束；也支援點一下開始/再點結束，行動裝置友善）。錄音同時進行（見 §4）。
...
| 發音信心 | 辨識 `confidence`：≥0.85 滿分，線性遞減，<0.5 得 0。 |
...
- 麥克風按下時同時啟動 `MediaRecorder`（`getUserMedia audio`）與 `webkitSpeechRecognition`。
